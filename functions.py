...
def train_decay(model, model_init, decay, device, train_loader, criterion, optimizer, epoch):
    ...
    for i, (data, target) in enumerate(train_loader):
        ...
        output = model(data)
        loss = criterion(output, target)
        diff = diff_norm(list(model.parameters()),
                         list(model_init.parameters()))
...
def FIM_true(model, criterion, loader, device, k):
    ...
    u = (grad_all.T / np.sqrt(len(loader))) @ v
    u = u * torch.sqrt(1/L)
    idx = list(np.flip(L.cpu().numpy().argsort()))
    L = L[idx]
    u = (u.T[idx]).T
...
def kfac_top_eigvec(kfac_list, model, k):
    ...
    tag1 = list_to_vec([torch.ones(p.shape)*i for i,
                        p in enumerate(model.parameters())])
...
def vhp(model, loader, w0_tuple, v_tuple, criterion, device, half=False):
    ...
    for (data, targets) in loader:
        ...
        def f(*param_tuple):
            loss = functional2(model, data, targets, criterion,
                               device, param_tuple, names_all)
            return loss
...
def hess_lanczo(model, k, loader, criterion, device, half=False):
    ...
    for i in range(k):
        print(i)
        _, v = vhp(model, loader, tuple(param_list),
                   tuple(q1), criterion, device)
        v = list(v)
        alpha.append(prod_list(q1, v))

        for (ele_v, ele_q0, ele_q1) in zip(v, q0, q1):
            ele_v.data = ele_v.data - beta[i] * \
                ele_q0.data - alpha[i+1]*ele_q1.data
...
def hess_scipy(model, k, loader, criterion, device):
    ...
    def fnc_LO(q):
        ...
        _, v_tuple = vhp(model, loader, tuple(param_list),
                         tuple(q_list), criterion, device)
...
def hess_FIM(model, criterion, loader_FIM, loader_hess, k, device):
    ...
    for i in range(k):
        ...
        _, v_tuple = vhp(model, loader_hess, tuple(
            param_list), tuple(q1), criterion, device)
...
def proj_FIM_kron(r, kfac, device):
    ...
    if len(kfac) == 2:
        ...
        eo, vo = torch.tensor(np.real(eo)).to(
            device), torch.tensor(np.real(vo)).to(device)
        ei, vi = torch.tensor(np.real(ei)).to(
            device), torch.tensor(np.real(vi)).to(device)

        # C = torch.kron(outmat, inmat)

        e = (torch.kron(eo.contiguous(), ei.contiguous())
             ).reshape(len(outmat), len(inmat))
    ...
    if len(kfac) == 1:
        ...
        e, v = torch.tensor(np.real(e)).to(
            device), torch.tensor(np.real(v)).to(device)
        r = r.to(device)
        mat = mat.to(device)
